...
def curriculum(opener,headers):
    c_data=''
    try:#课表爬虫
        get2_url = 'http://210.38.250.43/xsgrkbcx!xsAllKbList.action?xnxqdm=201901'
        request = urllib.request.Request(url=get2_url,headers=headers)
        response = opener.open(request)
        kb = response.read().decode()
        pattern = re.compile(r'var kbxx = (.*?)];')
...
def achievement(opener,headers):
    ...
    try:
        ...
        if (len(a_name) > 1):
            ...
            a_data = '[' + a_data + '{' + '"a_name":"' + a_name[n + 1] + '","a_point":"' + a_point[n + 1] + '","a_time":"' + a_time[n] + '","a_achievement":"' + a_achievement[n + 1] + '","a_type":"' + a_type[n + 1] + '","a_state":"' + a_state[n + 1] + '","a_credit":"' + a_credit[n + 1] + '"}' + ']'
        else:
